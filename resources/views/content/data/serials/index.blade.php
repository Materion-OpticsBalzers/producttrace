                ...
                <div class="bg-white flex flex-col px-2 py-2 shadow-sm">
                    ...
                    @if($order->po == '')
                        <form action="{{ route('serialise.store', ['order' => $order->id]) }}" method="POST" class="flex items-center gap-2 mt-1">
                            @csrf()
                            <input type="text" name="po" value="{{ old('po') }}" class="rounded-sm border-0 h-8 focus:ring-[#0085CA] font-semibold bg-gray-200" placeholder="AB">
                            <input type="text" name="po_pos" value="{{ old('pos_pos') }}" class="rounded-sm border-0 h-8 focus:ring-[#0085CA] font-semibold bg-gray-200" placeholder="POS">
                            <button type="submit" class="bg-[#0085CA] rounded-sm font-semibold text-white text-sm h-8 px-2 hover:bg-[#0085CA]/80">Setzen</button>
                        </form>
                    @else
                        <span class="text-sm">AB: {{ $order->po }} - POS: {{ $order->po_pos }}</span>
                    @endif
