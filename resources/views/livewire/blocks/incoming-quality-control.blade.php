            ...
            <div class="flex flex-col gap-2 mt-3" x-data="{ operator: {{ auth()->user()->personnel_number }}, boxId: '', rejection: 6 }">
                <div class="flex flex-col">
                    ...
                    <div class="flex flex-col w-full relative" x-data="{ show: false, search: '' }" @click.away="show = false">
                        <input type="text" wire:model.lazy="selectedWafer" @focus="show = true" autofocus class="w-full bg-gray-100 rounded-sm font-semibold text-sm border-0 focus:ring-[#0085CA]" placeholder="Wafer ID eingeben oder scannen..."/>
            ...
            <div class="flex flex-col gap-1 mt-2" wire:loading.remove.delay.longer wire:target="search">
                ...
                @forelse($wafers as $wafer)
                    <div class="bg-white border @if($wafer->rejection->reject) border-red-500/50 @else border-green-600/50 @endif flex flex-col rounded-sm hover:bg-gray-50 items-center" x-data="{ waferOpen: false, waferEdit: false }">
                        <div class="flex flex-col px-2 py-2 w-full" x-show="waferEdit" x-data="{ operator: '{{ $wafer->operator }}', box: '{{ $wafer->box }}', rejection: {{ $wafer->rejection_id }} }">
                            <div class="gap-1 grid grid-cols-4">
                                <input disabled type="text" value="{{ $wafer->wafer_id }}" class="bg-gray-200 rounded-sm border-0 focus:ring-[#0085CA] text-xs font-semibold"/>
                                <input x-model="operator" type="text" class="bg-gray-200 rounded-sm border-0 focus:ring-[#0085CA] text-xs font-semibold" tabindex="2" placeholder="Operator"/>
                                <input x-model="box" type="text" class="bg-gray-200 rounded-sm border-0 focus:ring-[#0085CA] text-xs font-semibold" tabindex="2" placeholder="Box ID"/>
                                <span class="bg-gray-200 rounded-sm w-full h-full text-xs flex items-center px-2 text-gray-500"><i class="fal fa-code mr-1"></i> Automatisch generiert...</span>
                            </div>
