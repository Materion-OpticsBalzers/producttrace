...
new class extends \Livewire\Volt\Component {
    ...
    public function checkWafer($waferId)
    {
        ...
        //\WaferHelper::waferDuplicatedCheck($waferId, $this->orderId);
    ...
    /**
     * Retrieves the wafers, rejections, and selected wafers based on certain conditions.
     *
     * @return array An associative array containing the wafers, rejections, and selected wafers.
     */
...
<div class="flex flex-col bg-gray-100 w-full h-full z-[9] border-l border-gray-200 overflow-y-auto" x-data>
    <div
        class="pl-8 pr-4 py-3 text-lg font-semibold shadow-sm flex border-b border-gray-200 items-center z-[8] sticky top-0 bg-white">
    ...
    <div class="h-full bg-gray-100 flex z-[7]" x-data="{ hidePanel: $persist(false) }"
        ...
        <div class="w-full h-max flex flex-col bg-white border-r border-gray-200 px-8 pt-3 pb-4" x-show="!hidePanel">
            ...
            <div class="flex flex-col h-full relative gap-2 mt-3"
                 x-data="{ operator: {{ auth()->user()->personnel_number }}, rejection: @entangle('selectedRejection') }">
                        ...
                        <div
                            ...
                            <div class="flex flex-col divide-y divide-gray-300" wire:loading.remove>
                                ...
                                @forelse($sWafers as $wafer)
                                    <a href="javascript:"
                                       wire:click="updateWafer('{{ $wafer->id }}', {{ $wafer->is_rework }})"
                                       @click="show = false"
                ...
                <div class="flex flex-col">
                    ...
                    <input wire:model="box" onfocus="this.setSelectionRange(0, this.value.length)" type="text"
                           class="bg-orange-100 @error('box') border-1 border-red-500/40 rounded-t-sm @else border-0 rounded-sm @enderror text-sm font-semibold"
                           tabindex="3" placeholder="Box ID"/>
                    @error('box')
                ...
                <div class="flex gap-2">
                    <button type="submit" @click="$wire.addEntry(operator, rejection)"
                            class="bg-[#0085CA] hover:bg-[#0085CA]/80 rounded-sm px-3 py-4 text-sm w-full uppercase text-white text-left"
                            tabindex="4">
        ...
        <div class="w-full px-4 py-3 flex flex-col h-full" x-show="hidePanel" x-cloak>
            ...
            <div class="flex flex-col gap-1 mt-2 pb-4" wire:loading.remove.delay.longer wire:target="search">
                <div
                    class="px-2 py-1 rounded-sm grid grid-cols-4 items-center justify-between bg-gray-200 shadow-sm mb-1">
                    ...
                    <div
                        ...
                        <div class="flex flex-col px-2 py-2 w-full" x-show="waferEdit" x-trap="waferEdit"
                            ...
                            <div class="flex gap-1 mt-2">
                                <a href="javascript:;"
                                   @click="$wire.updateEntry({{ $wafer->id }}, operator, box, rejection); waferEdit = false"
                        ...
                        <div class="flex flex-col w-full px-2 py-2 border-t bg-gray-50 border-gray-200 gap-1"
                             x-show="waferOpen && !waferEdit">
                            @error("rework{$wafer->id}") <span
                            ...
                            <div class="flex gap-1 mt-1 w-full">
                                ...
                                <a href="javascript:;"
                                   ...
                                   class="bg-red-500 right-0 text-xs px-3 py-1 uppercase hover:bg-red-500/80 rounded-sm text-white"><i
