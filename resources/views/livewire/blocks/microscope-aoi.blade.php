...
<div class="flex flex-col bg-gray-100 w-full h-full z-[9] border-l border-gray-200 overflow-y-auto" x-data="">
    ...
    <div class="h-full bg-gray-100 flex z-[7]" x-data="{ hidePanel: $persist(false) }" :class="hidePanel ? '' : 'flex-col'">
        ...
        <div class="w-full h-max bg-white flex flex-col border-r border-gray-200 px-8 pt-3 pb-4" x-show="!hidePanel">
            ...
            <div class="flex flex-col h-full relative gap-2 mt-3" x-data="{ operator: {{ auth()->user()->personnel_number }}, rejection: @entangle('rejection') }">
                ...
                <div class="flex flex-col">
                    ...
                    <div class="flex flex-col w-full relative" x-data="{ show: false, search: '' }" @click.away="show = false">
                        <div class="flex flex-col">
                            <div class="flex">
                                <input type="text" wire:model.live.debounce.500ms="selectedWafer" id="wafer" tabindex="1" onfocus="this.setSelectionRange(0, this.value.length)" @focus="show = true" class="w-full bg-gray-100 text-sm font-semibold @error('wafer') border-1 border-red-500/40 rounded-t-sm @else border-0 rounded-sm @enderror focus:ring-[#0085CA]" placeholder="Wafer ID eingeben oder scannen..."/>
                            </div>
                ...
                <div class="grid grid-cols-2 gap-2">
                    <div class="flex flex-col">
                        ...
                        <input wire:model="box" type="text" disabled class="bg-gray-100 text-sm font-semibold" tabindex="3" placeholder="Chrom Box ID"/>
                    </div>
                    <div class="flex flex-col">
                        <label class="text-sm mb-1 text-gray-500">AR Box ID *:</label>
                        <input wire:model="ar_box" onfocus="this.setSelectionRange(0, this.value.length)" type="text" class="bg-gray-100 @error('ar_box') border-1 border-red-500/40 rounded-t-sm @else border-0 rounded-sm @enderror text-sm font-semibold" tabindex="3" placeholder="AR Box ID"/>
                        @error('ar_box')
                        <div class="bg-red-500/20 text-red-500 flex items-center px-2 py-0.5 rounded-b-sm text-xs">
                            <i class="far fa-exclamation-circle mr-1 animate-pulse"></i>
                            <span class="font-semibold">{{ $message }}</span>
                ...
                <div class="flex flex-col gap-1">
                    <div class="grid grid-cols-2 gap-2">
                        <div class="flex flex-col">
                            ...
                            <input type="text" wire:model="cdo" class="mt-1 bg-gray-100 rounded-sm border-0 focus:ring-[#0085CA] text-sm font-semibold" placeholder="CD OL"/>
                        </div>
                        <div class="flex flex-col">
                            <label class="text-sm text-gray-500">CD UR:</label>
                            <input type="text" wire:model="cdu" class="mt-1 bg-gray-100 rounded-sm border-0 focus:ring-[#0085CA] text-sm font-semibold" placeholder="CD UR"/>
                        </div>
                    </div>
                    <div class="grid grid-cols-3 gap-1">
                        <div class="flex flex-col">
                            <label class="text-xs text-gray-500">X:</label>
                            <input type="text" wire:model="x" class="mt-1 bg-gray-100 rounded-sm border-0 focus:ring-[#0085CA] text-xs font-semibold" placeholder="X"/>
                        </div>
                        <div class="flex flex-col">
                            <label class="text-xs text-gray-500">Y:</label>
                            <input type="text" wire:model="y" class="mt-1 bg-gray-100 rounded-sm border-0 focus:ring-[#0085CA] text-xs font-semibold" placeholder="Y"/>
                        </div>
                        <div class="flex flex-col">
                            <label class="text-xs text-gray-500">Z:</label>
                            <input type="text" wire:model="z" class="mt-1 bg-gray-100 rounded-sm border-0 focus:ring-[#0085CA] text-xs font-semibold" placeholder="Z"/>
                        </div>
                    </div>
                    @if($format != null)
                ...
                <div class="flex gap-2">
                    @if($waferInfo && !$waferInfo->reworked)
                        <button type="submit" @click="$wire.addEntry(operator, true)" wire:key="reworkBtn" x-show="rejection != 6" class="bg-orange-500 w-max whitespace-nowrap hover:bg-orange-500/80 rounded-sm px-3 py-4 text-sm uppercase text-white text-left" tabindex="7">
                    ...
                    @endif
                    <button type="submit" @click="$wire.addEntry(operator)" class="bg-[#0085CA] w-full hover:bg-[#0085CA]/80 rounded-sm px-3 py-4 text-sm uppercase text-white text-left" tabindex="4">
