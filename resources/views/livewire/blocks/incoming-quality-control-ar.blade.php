<?php
    ...
    new class extends \Livewire\Volt\Component {
        ...
        public $forcedWaferSelection = false;
        ...
        public function updateWafer($wafer, $box, $forced = false) {
            $this->selectedWafer = $wafer;
            $this->box = $box;

            if($forced)
                $this->forcedWaferSelection = true;
        }
        ...
        public function with()
        {
            ...
            if($this->serial != '')
                $serials = Serial::where('order_id', $this->order->id)->where('id', 'like', "%{$this->serial}%")->where('rejected', false)->whereNull('wafer_id')->lazy();
            else
                $serials = Serial::where('order_id', $this->order->id)->where('rejected', false)->whereNull('wafer_id')->lazy();

            if($serials->count() > 0)
                $this->serial = $serials->first()->id;

            return compact(['wafers', 'rejections', 'sWafers', 'serials']);
        }
    }
?>
...
<div class="flex flex-col bg-gray-100 w-full h-full z-[9] border-l border-gray-200 overflow-y-auto" x-data="">
    ...
    <div class="h-full bg-gray-100 flex z-[7]" x-data="{ hidePanel: $persist(false) }" :class="hidePanel ? '' : 'flex-col'">
        ...
        <div class="w-full h-max flex flex-col bg-white border-r border-gray-200 px-8 pt-3 pb-4" x-show="!hidePanel">
            ...
            <div class="flex flex-col h-full relative gap-2 mt-3" x-data="{ operator: {{ auth()->user()->personnel_number }}, rejection: @entangle('selectedRejection') }">
                ...
                <div class="flex flex-col relative z-[6]" x-data="{ serialsOpen: false }">
                    ...
                    <input wire:model.live="serial" onfocus="this.setSelectionRange(0, this.value.length)" @focus="serialsOpen = true" @focusout="serialsOpen = false" type="text"
                           class="bg-orange-100 @error('serial') border-1 border-red-500/40 rounded-t-sm @else border-0 rounded-sm @enderror text-sm font-semibold" tabindex="2" placeholder="Serial eingeben oder scannen..."/>
                ...
                <div class="flex gap-2">
                    <button type="submit" @click="$wire.addEntry(operator, rejection)" class="bg-[#0085CA] w-full hover:bg-[#0085CA]/80 rounded-sm px-3 py-4 text-sm uppercase text-white text-left" tabindex="4">
        ...
        <div class="w-full px-4 py-3 flex flex-col pb-4 relative" x-show="hidePanel">
            <div class="flex w-full h-full justify-center items-center absolute z-[50]" wire:loading.flex>
                <div class="bg-white/50 w-full h-full absolute"></div>
                <span class="text-2xl text-[#0085CA] font-extrabold z-[51]"><i class="fal fa-spinner fa-spin"></i> Daten werden geladen...</span>
            </div>
            ...
            <div class="flex gap-4">
                <select wire:model="searchField" class="bg-white rounded-sm mt-2 mb-1 text-sm font-semibold w-max shadow-sm border-0 focus:ring-[#0085CA]">
                    ...
                    <option value="box">AR Box ID</option>
                </select>
                    ...
                    <div class="bg-white border @if($wafer->rejection->reject) border-red-500/50 @elseif($wafer->reworked || $wafer->wafer->reworked) border-orange-500/50 @else border-green-600/50 @endif flex flex-col rounded-sm hover:bg-gray-50 items-center" x-data="{ waferOpen: false, waferEdit: false }">
                        ...
                        <div class="flex w-full px-2 py-1 cursor-pointer items-center" @click="waferOpen = !waferOpen" x-show="!waferEdit">
                            ...
                            <div class="flex flex-col grow">
                                <div class="grid grid-cols-5 items-center">
                                    ...
                                    <span class="text-xs">{{ $wafer->serial_id }}</span>
                        ...
                        <div class="flex flex-col w-full px-2 py-2 border-t bg-gray-50 border-gray-200 gap-1" x-show="waferOpen && !waferEdit">
                            ...
                            <div class="flex gap-1 mt-1 w-full">
                                ...
                                <a href="javascript:;" onclick="confirm('Willst du diesen Wafer wirklich löschen?') || event.stopImmediatePropagation()" wire:click="removeEntry({{ $wafer->id }})" class="bg-red-500 right-0 text-xs px-3 py-1 uppercase hover:bg-red-500/80 rounded-sm text-white"><i class="fal fa-trash mr-1"></i> Wafer löschen</a>
                            </div>
                        </div>
                    </div>
